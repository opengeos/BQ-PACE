...
const PRODUCTS: Record<string, ProductConfig> = {
  "chl-a": {
    name: "Chlorophyll-a",
    csvUrl: "https://data.source.coop/giswqs/opengeos/PACE/Chl-a/files.csv",
    baseUrl: "https://data.source.coop/giswqs/opengeos/PACE/Chl-a/",
    colormap: "jet",
    rescale: [0, 30],
    label: "Chlorophyll-a",
    unit: "mg/m³",
  ...
  acdom440: {
    name: "aCDOM440",
    csvUrl: "https://data.source.coop/giswqs/opengeos/PACE/aCDOM440/files.csv",
    baseUrl: "https://data.source.coop/giswqs/opengeos/PACE/aCDOM440/",
    colormap: "jet",
    rescale: [0, 2],
    label: "aCDOM440",
    unit: "m⁻¹",
  ...
  tss: {
    ...
    csvUrl: "https://data.source.coop/giswqs/opengeos/PACE/TSS/files.csv",
    baseUrl: "https://data.source.coop/giswqs/opengeos/PACE/TSS/",
    colormap: "jet",
    rescale: [0, 50],
    label: "TSS",
    unit: "mg/L",
...
function getProductFromUrl(): ProductConfig {
  const params = new URLSearchParams(window.location.search);
  const productId = params.get("product") || "chl-a";
  return PRODUCTS[productId] || PRODUCTS["chl-a"];
}

const product = getProductFromUrl();

// Update page title
document.title = `PACE ${product.name} Viewer`;
...
async function fetchRasterData(): Promise<{ labels: string[]; urls: string[] }> {
  ...
  for (const filename of filenames) {
    ...
    if (date) {
      ...
      if (!rasterData[date]) {
        rasterData[date] = product.baseUrl + filename;
      }
    }
  }
...
const RASTER_SOURCE_ID = "pace-raster";
const RASTER_LAYER_ID = "pace-layer";

// Initialize the map with raster data
async function initializeMap() {
  const { labels, urls } = await fetchRasterData();
  console.log(`Loaded ${labels.length} PACE ${product.name} raster files`);
  ...
  const setupMap = () => {
    ...
    const colorbar = new Colorbar({
      colormap: product.colormap as any,
      vmin: product.rescale[0],
      vmax: product.rescale[1],
      label: `${product.label} (${product.unit})`,
    ...
    console.log(`PACE ${product.name} time slider control added to map`);
  };
